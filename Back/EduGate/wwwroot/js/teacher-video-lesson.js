/* Page script: pages/teacher/video-lesson.html */
/* ============================================================
...
populateSidebarUser();
  ...
  async function loadLesson() {
    ...
    setBreadcrumbTrail('page-breadcrumb', [
      { label: 'Home', href: '/Teacher/Dashboard' },
      { label: 'Courses', href: '/Teacher/Courses' },
      { label: course.title, href: `/Teacher/CourseDetails?id=${courseId}` },
      { label: lesson.title },
    ]);
      ...
      <div class="video-layout">
        <div>
          ...
          <div class="video-controls">
            ${prev ? `<button class="btn-secondary" onclick="window.location.href='/Teacher/VideoLesson?courseId=${courseId}&lessonId=${prev.id}'">Previous</button>` : ''}
            <button class="btn-amber" id="mark-done-btn" style="margin-left:auto">Mark as Complete</button>
            ${next
              ? `<button class="btn-primary" onclick="window.location.href='/Teacher/VideoLesson?courseId=${courseId}&lessonId=${next.id}'">Next Lesson</button>`
              : `<button class="btn-primary" onclick="window.location.href='/Teacher/CourseDetails?id=${courseId}'">Finish Course</button>`}
          </div>
        </div>
        ...
        <div class="lessons-sidebar">
          ...
          <div class="lessons-sidebar-list">
            ${lessons.map((item, i) => `
              <div class="sidebar-lesson ${item.id === lessonId ? 'active' : ''} ${item.completed ? 'completed' : ''}" onclick="window.location.href='/Teacher/VideoLesson?courseId=${courseId}&lessonId=${item.id}'">
    ...
    document.getElementById('mark-done-btn')?.addEventListener('click', async () => {
      ...
      submitMvcForm('/Teacher/Lessons/Complete', { CourseId: courseId, LessonId: lessonId });
